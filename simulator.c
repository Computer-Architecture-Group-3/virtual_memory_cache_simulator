...
int parse_dst_src_line(const char *line, 
                       unsigned long long *dst, int *dst_valid,
                       unsigned long long *src, int *src_valid)
{
    ...
    sscanf(line, "dstM: %s %s srcM: %s %s", dst_addr, dst_data,src_addr, src_data);
      ...
      for (int i = 0; i < fileCount; i++){
        ...
        while (1) {
          ...
          if (!fgets(line2, sizeof(line2), fp[i])) break;
            ...
            if (parse_dst_src_line(line2, &dst_addr, &dst_valid, &src_addr, &src_valid)) {
